...
function inWords(num) {
    ...
    str +=
        n[1] != 0
            ? (a[Number(n[1])] || b[n[1][0]] + " " + a[n[1][1]]) + "Trăm "
            : "";
    str +=
        n[2] != 0
            ? (a[Number(n[2])] || b[n[2][0]] + " " + a[n[2][1]]) + "Triệu "
            : "";
    str +=
        n[3] != 0
            ? (a[Number(n[3])] || b[n[2][0]] + " " + a[n[3][1]]) + "Trăm "
            : "";
    str +=
        n[4] != 0
            ? (a[Number(n[4])] || b[n[3][0]] + " " + a[n[4][1]]) + "Nghìn "
            : "";
    str +=
        n[5] != 0
            ? (a[Number(n[5])] || b[n[4][0]] + " " + a[n[5][1]]) + "Trăm "
            : "";
    str +=
        n[6] != 0
            ? (str != "" ? "" : "") +
...
router.get(
    ...
    asyncHandler(async function postLogin(req, res) {
        ...
        res.render("./pages/savingAccount/saving", {
            fund: fund,
            saving: itemSaving,
            errors: null,
        });
...
router.get(
    "/saving/addSaving",
    asyncHandler(async function postLogin(req, res) {
        userSavingAccount = await SavingAccount.findSavingAccountrByAccountNumber(
            req.session.account.accountNumber
        );
        userSavingAccount.forEach((x) => {
            fund = fund + parseInt(x.fund);
        });

        res.render("./pages/savingAccount/savingAccount", {
            fund: fund,
            errors: null,
        });
    })
...
router.get(
    ...
    asyncHandler(async function postLogin(req, res) {
        ...
        Email.send(
            ...
            bank17ez,
            "Mã xác thực tất toán  : " + tokenTatToan
        );
    ...
    asyncHandler(async function postLogin(req, res) {
        if (req.body.OTP.toUpperCase() == tokenTatToan) {
            ...
            await Account.updateBalance(
                extraMoney,
                res.locals.account.accountNumber
            );
            await SavingAccount.deleteById(id);
            Email.send(
                res.locals.currentUser.email,
                bank17ez,
                "Tất toán thành công số tiền  : " + itemSaving.fund
            );
...
router.get("/", async (req, res) => {
    ...
    subEmail =
        ...
        "****" +
        userEmail.substring(lengthEmail - 11, lengthEmail);
    ...
    if (userSavingAccount) {
        return res.render("./pages/savingAccount/listSaving", {
            fund: fund,
            saving: userSavingAccount,
            errors: null,
        });
    } else {
...
router.post(
    ...
    asyncHandler(async function (req, res) {
        ...
        if (!errors.isEmpty()) {
            return res
                .status(422)
                .render("./pages/savingAccount/savingAccount", {
                    fund: fund,
        ...
        if (!req.body.amountSaving) {
            ...
            if (req.body.OTP.toUpperCase() == token) {
                ...
                var closeDate = new Date(
                    dt.setMonth(dt.getMonth() + depositTerm)
                );
